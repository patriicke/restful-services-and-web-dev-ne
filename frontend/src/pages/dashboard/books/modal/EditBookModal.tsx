...
import { z } from 'zod';
import { update_book } from '~/api/books';
import { toast } from 'react-toastify';
import { CustomError } from '~/core/libs';
import { BookType, UpdateBookRequestPayload } from '~/core/types/book';
...
const UpdateBookModal: FC<UpdateBookModalProps> = props => {
    const { open, onClose, selectedBook, udpateBook } = props;
    const [isLoading, setIsLoading] = useState(false);

    const handleSubmit = async (payload: UpdateBookRequestPayload) => {
        try {
            setIsLoading(true);
            if (!selectedBook?.id) {
            ...
            if (_.isEqual(selectedBook, { ...selectedBook, ...payload })) {
                toast.warn('No changes made');
                return;
            }
            const data = await update_book(selectedBook.id, payload);
            udpateBook(data.payload.book);
    ...
    return (
        <Modal onClose={onClose} open={open}>
            <DialogPanel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                <Form<UpdateBookRequestPayload, typeof schema>
                    schema={schema}
                    ...
                    options={{
                        defaultValues: {
                            ...
                            publicationYear:
                                selectedBook?.publicationYear.toString() as any,
                        },
                    }}
                >
                    {({ register, formState }) => (
